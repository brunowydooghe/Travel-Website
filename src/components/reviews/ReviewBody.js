...
const ReviewBody = ({review}) => {
    const rating = (number) => {
        ...
        for(let i = 1; i <= 5; i++) {
            if(i <= number) {
                container.push(<BsFillStarFill key={i} size={13} color="#df2189"
            ...
            <div className="reviews__body animation">
                <div className="reviews__body__contents">
                    ...
                    <div className="reviews__body__contents__info">
                        ...
                        <div className="reviews__body__contents__info__rating">{rating(review.stars)}</div>
                        <div className="reviews__body__contents__info__comment">{review.comment}</div>
                    </div>
                </div>
            </div>
        </div>
